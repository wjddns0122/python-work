...
def playGame() :
    global monitor, ship, monster, missile, shipSize, r, g, b
    ...
    # @기능 4-6 : 우주선의 속도를 저장할 변수를 선언한다.
    shipSpeed = 5

    # @기능 5-5 : 우주선이 폭파된 후 폭발 효과를 보여주기 위한 변수를 선언한다.
    ...
    monsterSpeedX = random.choice([-9, -7, -5, 5, 7, 9]) # X축 속도 (지그재그)
    monsterSpeedY = random.randrange(1, 5) # Y축 속도
    ...
    # @기능 4-7 : 미사일의 속도를 저장할 변수를 선언한다.
    missileSpeed = 10

    # 무한 반복
    while True :
        (pygame.time.Clock()).tick(40)  # 게임 진행을 늦춘다(10~100 정도가 적당).
        monitor.fill((r, g, b))              # 화면 배경을 칠한다.

        if fireCount % 3 == 0 and fireCount > 0:
        # @기능 4-5 : 우주선의 속도를 상승시킨다.
            shipSpeed = min(10, shipSpeed + 1)
        ...
        for e in pygame.event.get() :
            ...
            if e.type in [pygame.KEYDOWN] :
                ...
                # @기능 4-3 : 스페이스바를 누르면 미사일을 발사한다.
                    ...
                    if missileX is None:  # 미사일을 쏜 적이 없다면
        ...
        # @기능 3-3 : 우주괴물이 자동으로 나타나 왼쪽에서 오른쪽으로 움직이고 지그재그로 내려온다.
        monsterX += monsterSpeedX
        monsterY += monsterSpeedY

        if monsterX > swidth or monsterX < 0:
            monsterSpeedX = -monsterSpeedX  # 좌우 방향 변경
        if monsterY > sheight * 0.3 or monsterY < 0:
            monsterSpeedY = -monsterSpeedY  # 상하 방향 변경
            
            # 우주괴물 이미지를 무작위로 선택한다.
            monster = pygame.image.load(random.choice(monsterImage))
            monsterSize = monster.get_rect().size
            monsterSpeedX = random.randrange(5, 9)
        ...
        if missileX is not None and (monsterX < missileX < monsterX + monsterSize[0]) and \
        (monsterY < missileY < monsterY + monsterSize[1]):
            ...
            monsterSpeedX = random.choice([-9, -7, -5, 5, 7, 9])
            monsterSpeedY = random.randrange(1, 5)

            # 미사일을 초기화한다.
            missileX, missileY = None, None  # 총알이 사라진다.

            # 우주선 속도/미사일 속도 상승
            monsterSpeedX = max(-9, monsterSpeedX - 1)
            monsterSpeedY = min(9, monsterSpeedY + 1)  
            shipSpeed = min(10, shipSpeed + 1)
            missileSpeed = min(20, missileSpeed + 2)

        # @기능 5-3 : 점수를 화면에 쓰는 함수를 호출한다. <== 117행 
        writeScore(fireCount)

        # 우주 괴물이 살아서 오른쪽 끝 또는 아래까지 도달할 경우, 점수에서 1점 차감
        if monsterX > swidth or monsterY > sheight * 0.3:
            fireCount -= 1
